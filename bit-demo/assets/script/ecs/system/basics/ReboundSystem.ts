...
export class ReboundSystem extends ecs.System {
    ...
    public update(_dt: number): void {
        ...
        const halfWidth = screen.windowSize.width / view.getScaleX() * 0.5;
        const halfHeight = screen.windowSize.height / view.getScaleY() * 0.5;

        for (const [_entity, position, direction, qtshape] of query.iterate3(Position, Direction, QTShape)) {
            const rect = qtshape.shape.getBoundingBox();
