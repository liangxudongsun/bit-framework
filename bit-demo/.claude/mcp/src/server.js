...
server.tool(
    ...
    {
        query: z.string().describe('搜索关键词，如"延迟执行"、"窗口显示"、"资源加载"'),
        source: z.enum(['framework', 'cocos', 'all']).default('all').describe('搜索范围'),
    },
    async ({ query, source }) => {
        const results = await searcher.search(query, source);
        if (results.length === 0) return { content: [{ type: 'text', text: '未找到相关API' }] };

        const text = results.map(cls =>
            `## ${cls.name}（${cls.module}）\n${cls.description}\n主要方法：${cls.methods.slice(0, 5).map(m => m.name).join(', ')}`
        ).join('\n\n');
...
server.tool(
    'get_module',
    '查询某个模块包含的所有类和接口',
    { moduleName: z.string().describe('模块名，如 bit-ui、bit-ecs、bit-event、cocos') },
    async ({ moduleName }) => {
