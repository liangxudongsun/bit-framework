...
function buildClassInfo(cls, moduleName) {
    ...
    const description = cls.getJsDocs().map(doc => doc.getDescription()).join(' ').trim();

    const mappedMethods = cls.getMethods().map(m => ({
        name: m.getName(),
        signature: m.getText().split('{')[0].trim(),
        description: m.getJsDocs().map(doc => doc.getDescription()).join(' ').trim(),
        searchText: `${m.getName()} ${m.getJsDocs().map(doc => doc.getDescription()).join(' ')}`,
    }));

    const mappedProperties = cls.getProperties().map(p => ({
        name: p.getName(),
        type: p.getType().getText(),
        description: p.getJsDocs().map(doc => doc.getDescription()).join(' ').trim(),
    }));
    ...
    return {
        ...
        extends: cls instanceof ClassDeclaration
            ? cls.getExtends()?.getExpression().getText()
            : (() => { const ifaces = cls.getExtends?.() ?? []; return ifaces.length > 0 ? ifaces.map(e => e.getExpression().getText()).join(', ') : undefined; })(),
        methods: mappedMethods,
        properties: mappedProperties,
        // searchText 包含方法名和属性名，使"延迟执行"能命中含 scheduleOnce 方法的类
        searchText: `${name} ${description} ${moduleName} `
...
function indexContainer(container, mod, indexData) {
    for (const cls of container.getClasses()) {
        ...
        if (indexData.classes[info.name]) {
            console.warn(`[WARN] 重名类: "${info.name}" 在模块 ${mod} 中重复，与模块 ${indexData.classes[info.name].module} 冲突，将被覆盖`);
        }
    ...
    for (const iface of container.getInterfaces()) {
        ...
        if (indexData.classes[info.name]) {
            console.warn(`[WARN] 重名接口: "${info.name}" 在模块 ${mod} 中重复，与模块 ${indexData.classes[info.name].module} 冲突，将被覆盖`);
        }
        indexData.classes[info.name] = info;
        if (!indexData.modules[mod]) indexData.modules[mod] = [];
        indexData.modules[mod].push(info.name);
    ...
    for (const func of container.getFunctions()) {
        ...
        const funcDesc = func.getJsDocs().map(doc => doc.getDescription()).join(' ').trim();
        const info = {
            name: funcName,
            module: mod,
        ...
        if (indexData.classes[funcName]) {
            console.warn(`[WARN] 重名函数: "${funcName}" 在模块 ${mod} 中与已有条目冲突，将被覆盖`);
        }
...
async function buildVectors(indexData, embedder) {
    ...
    for (let i = 0; i < classes.length; i++) {
        ...
        const output = await embedder(cls.searchText, { pooling: 'mean', normalize: true });
...
async function main() {
    ...
    if (!fs.existsSync(FRAMEWORK_DTS_DIR)) {
        ...
        const frameworkDtsPaths = fs.readdirSync(FRAMEWORK_DTS_DIR)
            .filter(f => f.endsWith('.d.ts'))
            .map(f => path.join(FRAMEWORK_DTS_DIR, f));
        buildIndex(frameworkDtsPaths, null, frameworkIndex);
    }
    fs.writeFileSync(path.join(INDEX_DIR, 'framework.json'), JSON.stringify(frameworkIndex, null, 2));
    console.log(`✅ framework.json: ${Object.keys(frameworkIndex.classes).length} 个类`);

    console.log('生成框架向量索引...');
    const frameworkVectors = await buildVectors(frameworkIndex, embedder);
    fs.writeFileSync(path.join(INDEX_DIR, 'framework-vectors.json'), JSON.stringify(frameworkVectors));
    ...
    fs.writeFileSync(path.join(INDEX_DIR, 'cocos.json'), JSON.stringify(cocosIndex, null, 2));
    ...
    fs.writeFileSync(path.join(INDEX_DIR, 'cocos-vectors.json'), JSON.stringify(cocosVectors));
